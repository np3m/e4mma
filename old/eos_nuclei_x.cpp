...
int eos_nuclei::solve_nuclei(size_t nv, const ubvector &x, ubvector &y,
  ...
  size_t n_nuclei=nuclei.size();
  if (T<=0.0 || n_nuclei!=1) {
    O2SCL_ERR("Must be only one nucleus for T=0",o2scl::exc_einval);
  }
  ...
  if (T<=0.0) {
    ...
    // In the zero-temperature limit, we can just use baryon number
    // conservation to determine the number of nuclei
    nuclei[i].n=(nB*Ye-np)/nuclei[i].Z;
    nn=nB-nuclei[i].Z*nuclei[i].n;
  ...
  // This loop determines the number densities of nuclei
  for(size_t i=0;i<n_nuclei;i++) {
